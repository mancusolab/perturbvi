...
def test_fit_screen_rejects_all_zero_g():
    X, _ = _make_data()
    G_bad = np.zeros((30, 3))
    screen = ScreenData(X=X, G=G_bad, gene_names=None, perturbation_names=None, cell_names=None, source={})
...
def test_fit_screen_rejects_row_mismatch():
    X, G = _make_data()
    screen = ScreenData(X=X, G=G[:20], gene_names=None, perturbation_names=None, cell_names=None, source={})
