...
def _make_screen(n=12, p=20, g=3):
    ...
    for i in range(n):
        G[i, i % g] = 1.0
    return ScreenData(X=X, G=G, genes=None, perturbations=None, cell_names=None, source={})
...
def test_x_not_2d_raises():
    screen = ScreenData(
        ...
        genes=None, perturbations=None, cell_names=None, source={}
    )
...
def test_genes_mismatch_raises():
    s = _make_screen(p=20)
    screen = ScreenData(
        X=s.X, G=s.G,
        genes=["g"] * 5,  # wrong: should be 20
        perturbations=None, cell_names=None, source={}
    )
    with pytest.raises(ValueError, match="genes"):
        validate_screen(screen)


def test_perturbations_mismatch_raises():
    s = _make_screen(g=3)
    screen = ScreenData(
        X=s.X, G=s.G,
        genes=None,
        perturbations=["p1", "p2"],  # wrong: should be 3
