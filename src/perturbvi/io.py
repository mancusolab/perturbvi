...
def find_top_genes(results: InferResults, pip_cutoff: float = 0.9, 
                  gene_symbol: Optional[list] = None, filepath: Optional[str] = None) -> dict:
...
# plot beta 
def plot_beta(results: InferResults, factor_idx: int, perturb_names: Optional[list] = None, n: int = 20, 
             figsize: Tuple[int, int] = (10, 6), fontsize: int = 8, 
    ...
    plt.scatter([numeric_indices[df.index.get_loc(i)] for i in top_indices],
                df.loc[top_indices, column], color='red', zorder=5)
